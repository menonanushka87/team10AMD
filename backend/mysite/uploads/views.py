...
def process_files_standalone( sarif_filename):
...
def upload_file(request):
    if request.method == 'POST' and request.FILES.get('sarifFile'):
        ...
        # Process files
        analysis_results_json = process_files_standalone(sarif_file.name)
        analysis_results = json.loads(analysis_results_json) 

        return JsonResponse(analysis_results)
    else:
        return JsonResponse({'error': 'SARIF file is required'}, status=400)
